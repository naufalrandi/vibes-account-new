...
describe("isra taxonomy + asset library (F-2a)", () => {
  ...
  it("CRUDs PA groups and sub-groups, blocking deletion while referenced", async () => {
    ...
    const group = await request(app).post("/v1/isra-library/taxonomy/pa-groups").set(authed(token)).send({ name: "Documents and Records" });
    expect(group.status).toBe(201);
    expect(group.body.data.id).toMatch(/^PAG-\d{3}$/);
    const groupId = group.body.data.id;

    const sub = await request(app).post("/v1/isra-library/taxonomy/pa-subgroups").set(authed(token))
    ...
    const list = await request(app).get(`/v1/isra-library/taxonomy/pa-subgroups?groupId=${groupId}`).set(authed(token));
    expect(list.body.data).toHaveLength(1);

    const delGroup = await request(app).delete(`/v1/isra-library/taxonomy/pa-groups/${groupId}`).set(authed(token));
    expect(delGroup.status).toBe(409);
    expect(delGroup.body.error.code).toBe("PA_GROUP_IN_USE");

    const delSub = await request(app).delete(`/v1/isra-library/taxonomy/pa-subgroups/${subId}`).set(authed(token));
    expect(delSub.status).toBe(200);
    const delGroup2 = await request(app).delete(`/v1/isra-library/taxonomy/pa-groups/${groupId}`).set(authed(token));
    expect(delGroup2.status).toBe(200);
  });

  it("runs the SA Subgroup approval-workflow transition", async () => {
    const token = await soLogin();
    const group = await request(app).post("/v1/isra-library/taxonomy/sa-groups").set(authed(token)).send({ name: "Applications and Software" });
    const groupId = group.body.data.id;
    expect(groupId).toMatch(/^SAG-\d{3}$/);
    const sub = await request(app).post("/v1/isra-library/taxonomy/sa-subgroups").set(authed(token))
    ...
    const badStatus = await request(app).post(`/v1/isra-library/taxonomy/sa-subgroups/${subId}/status`).set(authed(token)).send({ status: "Bogus" });
    expect(badStatus.status).toBe(400);
    expect(badStatus.body.error.code).toBe("INVALID_STATUS");

    const underReview = await request(app).post(`/v1/isra-library/taxonomy/sa-subgroups/${subId}/status`).set(authed(token)).send({ status: "Under review" });
    expect(underReview.body.data.status).toBe("Under review");

    const approved = await request(app).post(`/v1/isra-library/taxonomy/sa-subgroups/${subId}/status`).set(authed(token)).send({ status: "Approved" });
    expect(approved.body.data.status).toBe("Approved");

    const retired = await request(app).post(`/v1/isra-library/taxonomy/sa-subgroups/${subId}/status`).set(authed(token)).send({ status: "Retired" });
  ...
  it("forbids non-Service-Owner actors from taxonomy/catalogue admin mutations even with the admin grant", async () => {
    ...
    const res = await request(app).post("/v1/isra-library/taxonomy/pa-groups").set(authed(token)).send({ name: "X" });
    expect(res.status).toBe(403);
  });

  it("allows any authenticated org to read the global taxonomy and asset catalogue", async () => {
    const soToken = await soLogin();
    await request(app).post("/v1/isra-library/taxonomy/pa-groups").set(authed(soToken)).send({ name: "Business Records" });
    const { token } = await makeTenant("t2", "T2", [ACTIONS.ISRA_LIBRARY_READ]);
    const list = await request(app).get("/v1/isra-library/taxonomy/pa-groups").set(authed(token));
  ...
  it("CRUDs the Primary/Secondary Asset Library with group/sub-group validation", async () => {
    const token = await soLogin();
    const paGroup = await request(app).post("/v1/isra-library/taxonomy/pa-groups").set(authed(token)).send({ name: "Business Records" });
    const paSub = await request(app).post("/v1/isra-library/taxonomy/pa-subgroups").set(authed(token))
      .send({ groupId: paGroup.body.data.id, name: "Financial Records" });

    const mismatch = await request(app).post("/v1/isra-library/catalog/primary-assets").set(authed(token))
      .send({ name: "Customer Ledger", groupId: paGroup.body.data.id, subgroupId: "PASG-999" });
    expect(mismatch.status).toBe(404);

    const asset = await request(app).post("/v1/isra-library/catalog/primary-assets").set(authed(token))
    ...
    const saGroup = await request(app).post("/v1/isra-library/taxonomy/sa-groups").set(authed(token)).send({ name: "Storage and Backup" });
    const otherSaGroup = await request(app).post("/v1/isra-library/taxonomy/sa-groups").set(authed(token)).send({ name: "Cloud and Virtual Infrastructure" });
    const saSub = await request(app).post("/v1/isra-library/taxonomy/sa-subgroups").set(authed(token))
      .send({ groupId: saGroup.body.data.id, name: "Backup Media" });

    const saMismatch = await request(app).post("/v1/isra-library/catalog/secondary-assets").set(authed(token))
    ...
    const saAsset = await request(app).post("/v1/isra-library/catalog/secondary-assets").set(authed(token))
    ...
    const updated = await request(app).put(`/v1/isra-library/catalog/secondary-assets/${saAsset.body.data.id}`).set(authed(token))
      .send({ description: "Offline archival tape storage" });
    expect(updated.body.data.description).toBe("Offline archival tape storage");

    const deleted = await request(app).delete(`/v1/isra-library/catalog/secondary-assets/${saAsset.body.data.id}`).set(authed(token));
...
describe("isra Lt override/item/archive/audit system (F-2a)", () => {
  ...
  async function seedPlatformPrimaryAsset(token: string): Promise<{ id: string; name: string }> {
    const group = await request(app).post("/v1/isra-library/taxonomy/pa-groups").set(authed(token)).send({ name: "Business Records" });
    const sub = await request(app).post("/v1/isra-library/taxonomy/pa-subgroups").set(authed(token))
  ...
  it("customizes a platform item via override, then restores the platform default", async () => {
    ...
    const effBefore = await request(app).get("/v1/isra-library/lt/primary/effective").set(authed(token));
    ...
    const save = await request(app).put(`/v1/isra-library/lt/primary/overrides/${asset.id}`).set(authed(token))
    ...
    const effAfter = await request(app).get("/v1/isra-library/lt/primary/effective").set(authed(token));
    ...
    const resave = await request(app).put(`/v1/isra-library/lt/primary/overrides/${asset.id}`).set(authed(token))
      .send({ name: "Customer Ledger (Localized)" });
    expect(resave.body.data.overrideVersion).toBe(2);
    expect(resave.body.data.history).toHaveLength(1);

    const restore = await request(app).delete(`/v1/isra-library/lt/primary/overrides/${asset.id}`).set(authed(token));
    expect(restore.status).toBe(200);
    const effRestored = await request(app).get("/v1/isra-library/lt/primary/effective").set(authed(token));
    expect(effRestored.body.data[0]).toMatchObject({ customized: false, name: asset.name });

    const restoreAgain = await request(app).delete(`/v1/isra-library/lt/primary/overrides/${asset.id}`).set(authed(token));
    expect(restoreAgain.status).toBe(404);

    const audit = await request(app).get("/v1/isra-library/lt/audit/log?libType=primary").set(authed(token));
  ...
  it("creates, copies, updates, archives and unarchives tenant-owned library items", async () => {
    ...
    const created = await request(app).post("/v1/isra-library/lt/primary/items").set(authed(token)).send({ name: "Wholly Custom Asset" });
    expect(created.status).toBe(201);
    expect(created.body.data.tenantItemId).toMatch(/^TPA-\d{4}$/);

    const eff = await request(app).get("/v1/isra-library/lt/primary/effective").set(authed(token));
    expect(eff.body.data).toHaveLength(2);
    const platformRow = eff.body.data.find((r: { source: string }) => r.source === "platform");

    const copied = await request(app).post("/v1/isra-library/lt/primary/items/copy").set(authed(token)).send({ sourceKey: platformRow.key });
    ...
    const updated = await request(app).put(`/v1/isra-library/lt/primary/items/${created.body.data.tenantItemId}`).set(authed(token))
      .send({ name: "Renamed Custom Asset" });
    expect(updated.status).toBe(200);
    expect(updated.body.data.name).toBe("Renamed Custom Asset");

    const effFinal = await request(app).get("/v1/isra-library/lt/primary/effective").set(authed(token));
    ...
    const archived = await request(app).post("/v1/isra-library/lt/primary/archive").set(authed(token)).send({ itemKey: tenantRow.key });
    expect(archived.status).toBe(201);
    const dupArchive = await request(app).post("/v1/isra-library/lt/primary/archive").set(authed(token)).send({ itemKey: tenantRow.key });
    expect(dupArchive.status).toBe(409);
    expect(dupArchive.body.error.code).toBe("ALREADY_ARCHIVED");

    const archList = await request(app).get("/v1/isra-library/lt/primary/archive").set(authed(token));
    expect(archList.body.data).toHaveLength(1);

    const effArchived = await request(app).get("/v1/isra-library/lt/primary/effective").set(authed(token));
    expect(effArchived.body.data.find((r: { key: string }) => r.key === tenantRow.key).archived).toBe(true);

    const unarchived = await request(app).post("/v1/isra-library/lt/primary/unarchive").set(authed(token)).send({ itemKey: tenantRow.key });
    expect(unarchived.status).toBe(200);
    const archListAfter = await request(app).get("/v1/isra-library/lt/primary/archive").set(authed(token));
    expect(archListAfter.body.data).toHaveLength(0);

    const missingUnarchive = await request(app).post("/v1/isra-library/lt/primary/unarchive").set(authed(token)).send({ itemKey: tenantRow.key });
  ...
  it("isolates Lt customizations and audit entries between orgs", async () => {
    ...
    const t1Save = await request(app).put(`/v1/isra-library/lt/primary/overrides/${asset.id}`).set(authed(t1.token)).send({ name: "Org1 Name" });
    expect(t1Save.status).toBe(200);

    const eff2 = await request(app).get("/v1/isra-library/lt/primary/effective").set(authed(t2.token));
    expect(eff2.body.data[0]).toMatchObject({ customized: false, name: asset.name });

    const audit1 = await request(app).get("/v1/isra-library/lt/audit/log").set(authed(t1.token));
    expect(audit1.body.data).toHaveLength(1);
    const audit2 = await request(app).get("/v1/isra-library/lt/audit/log").set(authed(t2.token));
  ...
  it("rejects an invalid library type and requires authentication", async () => {
    ...
    const bad = await request(app).get("/v1/isra-library/lt/bogus/effective").set(authed(token));
    expect(bad.status).toBe(400);
    expect(bad.body.error.code).toBe("INVALID_LIB_TYPE");

    const noAuth = await request(app).get("/v1/isra-library/lt/primary/effective");
