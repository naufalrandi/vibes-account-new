...
/**
 * List the caller's own organization's signatories, newest first. The org filter
 * is always taken from the authenticated context, never from request input.
 */
...
/** Resolve a single signatory owned by the actor's org, or 404. */
...
/** Flip a signatory between Active and Inactive (the row's activate/deactivate toggle). */
export async function toggleSignatory(auth: AuthContext, id: string, ip: string | null): Promise<OrgSignatory> {
  const sig = await requireOwned(auth, id);
  ...
  await writeAudit({
    ...
    organizationId: auth.orgId,
    tenantId: auth.tenantId,
    action: sig.status === "Active" ? "signatory.activated" : "signatory.deactivated",
    entityType: "OrgSignatory",
    entityId: sig.id,
    sourceIp: ip,
    result: "Success",
