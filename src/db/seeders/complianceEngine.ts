 ...
 * requirement catalogues, the 31 canonical FWEs, the CQ/CQR library, OD's 6
 * requirement criteria, and the 6,006 FWRC rows fanned out from OD's authored
 * statement templates by `seedClauseFWE`.
 *
 * Data modules (complianceEngine.*.data.ts) are GENERATED by booting the OD
 * prototype headlessly and reading its own seeded `db` — nothing is
 * hand-transcribed. Regenerate with:
 *
...
/**
 * The 31 canonical FWEs, keyed by their stable FWE-NNN code. Wrong names from
